...
const addSku = async (spu: SpuRecord) => {
  scene.value = 4
  await skuFormCom.value?.initAddSkuData(categoryStore.c1Id!, categoryStore.c2Id!, categoryStore.c3Id!, spu);
}
...
<template>
  <div class="spu">
    ...
    <el-card style="margin-top: 10px">
      <div v-show="scene == 2">
        <el-button type="primary" icon="plus" :disabled="categoryStore.c3Id == undefined" @click="addSpu">
          添加SPU
        </el-button>
        <el-table :data="categoryStore.spuPagedList?.records" style="width: 100%; margin: 10px 0" border>
          ...
          <el-table-column label="SPU操作" v-slot="{ row }">
            ...
            <el-button type="info" size="small" icon="View" title="查看SKU列表" @click="getSkuList(row.id)"></el-button>
            <el-popconfirm :title="`你确定要删除${row.spuName}吗?`" width="auto" @confirm="delSpu(row.id)">
              <template #reference>
        ...
        <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[3, 6, 9, 12]"
          ...
          :total="categoryStore.spuPagedList?.total ?? 0" @size-change="sizeChange" @current-change="getSPUPagedList" />
      </div>
      <SpuForm ref="spuFormCom" v-show="scene == 3" v-model="scene" v-model:current-page="currentPage" />
      <SkuForm ref="skuFormCom" v-show="scene == 4" @changeScene="changeScene" />
