...
import type { AddSkuData, SkuAttrValue, SkuSaleAttrValue } from '@/apis/product/sku/type';
...
const initAddSkuData = async (c1Id: number, c2Id: number, c3Id: number, spu: SpuRecord) => {
  let spuId = spu.id!;
...
const saveSku = async () => {
  ...
  skuFromData.value.skuAttrValueList = attrList.value.reduce((prev: SkuAttrValue[], curr: Attribute) => {
  ...
  skuFromData.value.skuSaleAttrValueList = spuSaleAttrList.value.reduce((prev: SkuSaleAttrValue[], curr: SpuSaleAttr) => {
    if (curr.attrIdAndValueId) {
  ...
  <div class="skuForm">
    <el-form label-width="100px" label-position="left" :model="skuFromData">
      ...
      <el-form-item label="价格（元）">
        <el-input placeholder="价格（元）" type="number" v-model="skuFromData.price" />
      </el-form-item>
      <el-form-item label="重量（g）">
        <el-input placeholder="重量（g）" type="number" v-model="skuFromData.weight" />
      </el-form-item>
      <el-form-item label="SKU描述">
        <el-input placeholder="SKU描述" type="textarea" v-model="skuFromData.skuDesc" />
      </el-form-item>
      <el-form-item label="平台属性">
        <el-form inline>
          <el-form-item v-for="attr in attrList" :key="attr.id" :label="attr.attrName">
            <el-select :placeholder="`请选择${attr.attrName}`" style="width: 150px" v-model="attr.attrIdAndValueId">
              <el-option v-for="val in attr.attrValueList" :key="val.id" :label="val.valueName"
      ...
      <el-form-item label="销售属性">
        <el-form inline>
          <el-form-item v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id" :label="saleAttr.saleAttrName">
            <el-select :placeholder="`请选择${saleAttr.saleAttrName}`" style="width: 150px"
              v-model="saleAttr.attrIdAndValueId">
              <el-option v-for="val in saleAttr.spuSaleAttrValueList" :key="val.id" :label="val.saleAttrValueName"
      ...
      <el-form-item label="图片名称">
        <el-table style="width: 100%" border :data="spuImageList" ref="imgTable">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="$emit('changeScene', 2)">取消</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
