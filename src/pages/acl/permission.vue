...
import { type MenuData, MenuFormData, type MenuItem } from '@/apis/acl/menu/type';
...
const editMenuHandler = (menu: MenuItem) => {
  ...
  Object.assign(menuFormData.value, new MenuFormData(id, code, level, name, pid))
}
  ...
  <div class="permission">
    <el-table :data="menuList" style="width: 100%; " row-key="id" border default-expand-all>
      ...
      <el-table-column label="操作" width="220px" v-slot="{ row }">
        <el-button type="primary" size="small" :disabled="row.level == 4" @click="addMenuHandler(row)">{{ row.level == 3
          ?
          '添加功能' : '添加菜单'
          }}</el-button>
        <el-button type="warning" size="small" :disabled="row.level == 1" @click="editMenuHandler(row)">编辑</el-button>
        <el-popconfirm :title="`你确定要删除【${row.name}】吗？`" width="auto" @confirm="delMenu(row.id)">
          <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="menuFormData.id ? '修改菜单' : '添加菜单'" width="500">
      <el-form>
        <el-form-item label="名称">
          <el-input placeholder="请你输入菜单名称" v-model="menuFormData.name"></el-input>
        </el-form-item>
        <el-form-item label="权限">
          <el-input placeholder="请你输入权限数值" v-model="menuFormData.code"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="dialogVisible = false">取消</el-button>
          <el-button type="primary" @click="upsertMenu">
            确定
          </el-button>
        </div>
      </template>
