...
import { reqGetPagedRoleList, reqUpsertRole, reqDeleteRole } from '@/apis/acl/role'
import { PagedRolesResponse, type Role } from '@/apis/acl/role/type'
import { ElMessageBox, type FormRules, type FormInstance, ElMessage, type TreeInstance } from 'element-plus'
...
const getPagedRoleList = async (page = 1) => {
  ...
  pagedRolesObj.value = await reqGetPagedRoleList(currentPage.value, pageSize.value, keyWord.value);
}
...
const setRolePermission = async () => {
  ...
  await reqSetRolePermission(roleFormData.value.id!, permissionIdList as number[]);
  ElMessage({
    type: 'success',
    message: '分配权限成功'
  });
...
const deleteRole = async (roleId: number) => {
  ...
  await getPagedRoleList(pagedRolesObj.value.records.length <= 1 && currentPage.value > 1 ? currentPage.value - 1 : currentPage.value)
}
</script>
  ...
  <div class="role">
    <el-card>
      <el-form label-width="80px" inline class="searchForm">
        ...
        <el-form-item>
          <el-button type="primary" :disabled="!keyWord" @click="getPagedRoleList()">搜索</el-button>
          <el-button type="success" @click="reset">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card style="margin-top: 10px;">
      <el-button type="primary" icon="plus" @click="addRoleHandler">添加职位</el-button>
      <el-table style="width: 100%;margin: 10px 0;" border :data="pagedRolesObj.records">
        ...
        <el-table-column label="操作" v-slot="{ row }" width="270px">
          <el-button type="primary" icon="user" size="small" @click="assignPermissionHandler(row)">
            分配权限
          </el-button>
          <el-button type="warning" icon="edit" size="small" @click="editRoleHandler(row)">
            编辑
          </el-button>
          <el-popconfirm :title="`你要删除${row.roleName}吗?`" width="auto" @confirm="deleteRole(row.id)">
            <template #reference>
      ...
      <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[3, 6, 9, 12,]"
        size="default" background layout="prev, pager, next, jumper, -> , sizes, total" :total="pagedRolesObj.total"
        @current-change="getPagedRoleList" @size-change="getPagedRoleList()" />
    </el-card>
    <el-dialog v-model="dialogVisible" :title="(roleFormData.id ? '修改' : '添加') + '职位'" :before-close="handleClose">
      <el-form label-width="80px" ref="roleForm" :model="roleFormData" :rules="rules">
        <el-form-item label="职位名称" prop="roleName">
          <el-input placeholder="请输入职位名称" v-model="roleFormData.roleName"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <div>
    ...
    <el-drawer v-model="drawerRole" title="分配菜单与按钮的权限" size="40%">
      <el-tree style="max-width: 600px" :data="menuData" show-checkbox node-key="id" default-expand-all
        :default-checked-keys="selectBtnPermissionIdList" :props="defaultProps" ref="permissionTree" />
      <template #footer>
        <div style="flex: auto">
